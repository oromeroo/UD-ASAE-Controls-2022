...
# define SEALEVELPRESSURE_SEA (1013.25)
#define SEALEVELPRESSURE_DEL (1024.8) //Found range for Delaware to be 1023-1026 milibars
...
String data_collection(){
  ...
  if (runcount < 5){
    initHeight = bmp.readAltitude(SEALEVELPRESSURE_DEL)*3.281; // Meters to feet -> multiply by 3.28084
  }
  data_1 = ((bmp.readAltitude(SEALEVELPRESSURE_DEL)*3.281)-initHeight);
  //data_1 = bmp.readAltitude(SEALEVELPRESSURE_DEL) * 3.28084;
  ...
  if(millis()-timer > 2000){
    timer = millis();
    if(GPS.fix){
      data_8 = GPS.latitudeDegrees * 1000000000;    // Latitude Data, 9 decimal points
      data_9 = GPS.longitudeDegrees * 1000000000;   // Longitude Data, 9 decimal points
      data_10 = GPS.speed * 1.15078;    // To convert knots to mph
    }
  }
  data_11 = 1.0; //Drop value is initially 1

  if(dropConfirm == true) {
    data_11 = 0.0; //0 marks pada release
  }
...
void loop() {
  ...
  // Serial.println(strncmp((char*)recvDataPacket,"drop",4));
